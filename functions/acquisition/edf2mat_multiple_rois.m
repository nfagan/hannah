    ...
    for ds = 1:length(doses) 
        ...
        for edfs = 1:length(edf_dir)
            ...
            if ~session_id_exists_in_mat_file || params.loadEdfIfMatExists;
                ...
                gaze = cell(size(img_file_names));
                
                for i = 1:length(regions)
                    for k = 1:length(img_file_names)
                        rois.(regions{i}){k} = ...
                            looking_coordinates_mult_images(img_file_names{k},monkeys{monks},regions{i},coordinates_and_labels);
                        if i == 1
                            gaze{k} = get_property_gaze(img_file_names{k},coordinates_and_labels);
                        end
                    end
                end
                ...
                empty_gaze = cellfun(@isempty,gaze);
                gaze(empty_gaze) = {0};
                ...
                image_data.data.gaze = concatenateData(gaze);

                % - 
                % data labeling -- n.b.: for flexibility, save gaze as both
                % data and labels
                % -
                ...
                image_data.labels.gaze = cellfun(@(x) num2str(x),gaze,'UniformOutput',false);
