function data = get_fix_event_data_faster(processed_image_data,data_labels)
...
data = struct(...
    ...
    'x_y_coords',[], ...
    'time',[], ...
    'per_image_pupil_size',[] ...
);
data_fields = fieldnames(data);

% - preallocate labels and data

for i = 1:length(data_fields)
    data.(data_fields{i}) = struct('data',[],'labels',[]);
    data.(data_fields{i}).data = cell(100000,1); %  arbitrarily huge
    for j = 1:length(label_fields)
        data.(data_fields{i}).labels.(label_fields{j}) = cell(10000,1);
    end
end
...
for i = 1:length(image_times);
    ...
    for j = 1:size(one_session_times,1); %for each image display time ...
        ...
        if one_session_times(j,2) < max(fix_ends); % if valid display time
            ...
            if ~isempty(all_durs)
                data.looking_duration.data{stp} = sum(all_durs);
                data.fix_event_duration.data{stp} = all_durs;
                data.successful_trial.data{stp} = 1;
                data.n_fixations.data{stp} = length(all_durs);
                data.x_y_coords.data{stp} = [allX(checkBounds) allY(checkBounds)];
                data.time.data{stp} = [repmat(startEndTimes,size(fixEvents(checkBounds,:),1),1) fixEvents(checkBounds,:)];
            else
                data.looking_duration.data{stp} = 0;
                data.fix_event_duration.data{stp} = 0;
                data.successful_trial.data{stp} = 0;
                data.n_fixations.data{stp} = 0;
                data.x_y_coords.data{stp} = [NaN NaN];
                data.time.data{stp} = [0 0 0 0];
            end
            ...
            data.per_image_pupil_size.data{stp} = one_image_pupil_size; % store for each image
            
            %   store labels
            
            for lab = 1:length(label_fields)
                data.looking_duration.labels.(label_fields{lab}){stp} = ...
                    repmat({data_labels.(label_fields{lab}){i}{j}},...
                    length(data.looking_duration.data{stp}),1);
                
                data.fix_event_duration.labels.(label_fields{lab}){stp} = ...
                    repmat({data_labels.(label_fields{lab}){i}{j}},...
                    length(data.fix_event_duration.data{stp}),1);
                
                data.successful_trial.labels.(label_fields{lab}){stp} = ...
                    repmat({data_labels.(label_fields{lab}){i}{j}},...
                    length(data.successful_trial.data{stp}),1);
                
                data.n_fixations.labels.(label_fields{lab}){stp} = ...
                    repmat({data_labels.(label_fields{lab}){i}{j}},...
                    length(data.n_fixations.data{stp}),1);
                
                data.per_image_pupil_size.labels.(label_fields{lab}){stp} = ...
                    repmat({data_labels.(label_fields{lab}){i}{j}},...
                    length(data.per_image_pupil_size.data{stp}),1);
                
                data.x_y_coords.labels.(label_fields{lab}){stp} = ...
                    repmat({data_labels.(label_fields{lab}){i}{j}},...
                    size(data.x_y_coords.data{stp},1),1);
                
                data.time.labels.(label_fields{lab}){stp} = ...
                    repmat({data_labels.(label_fields{lab}){i}{j}},...
                    size(data.time.data{stp},1),1);
            end
            stp = stp + 1;
        end
...
for i = 1:length(data_fields)
    empty_ind = cellfun(@isempty,data.(data_fields{i}).data);
    data.(data_fields{i}).data(empty_ind) = [];
    for k = 1:length(label_fields)
        data.(data_fields{i}).labels.(label_fields{k})(empty_ind) = [];
    end
end
